...
/**
 * Only start searching 500ms after the last input event
 */
function searchAfterDelay(dataCenter, err, inputElement, sr, maxResults, autoComplete) {
  clearTimeout(inputElement.searchTimeout)

  inputElement.searchTimeout = setTimeout(
    () => handleSearch(dataCenter, err, inputElement, sr, maxResults, autoComplete),
    500
  )
}
 ...
 */
function handleSearch(dataCenter, err, inputElement, sr, maxResults, autocomplete) {
  ...
  if (dataCenter) {
    ...
    if (inputElement.value != text) return;
...
DeclarationDataCenter.init()
  .then((dataCenter) => {
    ...
    SEARCH_INPUT.addEventListener("input", ev => 
      searchAfterDelay(dataCenter, null, ev.target, ac_results, AC_MAX_RESULTS, true)
    );
    if(SEARCH_PAGE_INPUT) {
      SEARCH_PAGE_INPUT.addEventListener("input", ev => 
        searchAfterDelay(dataCenter, null, ev.target, SEARCH_RESULTS, SEARCH_PAGE_MAX_RESULTS, false)
      );
      document.querySelectorAll(".kind_checkbox").forEach((checkbox) =>
  ...
  .catch(e => {
    SEARCH_INPUT.addEventListener("input", ev => 
      searchAfterDelay(null, e, ev.target, ac_results, AC_MAX_RESULTS,true )
    );
    if(SEARCH_PAGE_INPUT) {
      SEARCH_PAGE_INPUT.addEventListener("input", ev => 
        searchAfterDelay(null, e, ev.target, SEARCH_RESULTS, SEARCH_PAGE_MAX_RESULTS, false)
      );
    }
  });
